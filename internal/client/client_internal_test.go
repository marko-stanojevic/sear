...
func TestSaveLoadStateRoundTrip(t *testing.T) {
	...
	cfg := &common.ClientConfig{StateFile: stateFile}
	c := New(cfg)
	...
	c2 := New(&common.ClientConfig{StateFile: stateFile})
...
func TestPost(t *testing.T) {
	...
	cfg := &common.ClientConfig{ServerURL: ts.URL}
	c := New(cfg)
...
func TestConnectUnauthorizedClearsState(t *testing.T) {
	...
	stateFile := filepath.Join(t.TempDir(), "state.json")
	c := New(&common.ClientConfig{ServerURL: ts.URL, StateFile: stateFile})
	c.state.ClientID = "c1"
	c.state.Token = "t1"
...
func TestRunPlaybookMessageFlows(t *testing.T) {
	...
	c := New(&common.ClientConfig{ServerURL: "http://example.com"})
